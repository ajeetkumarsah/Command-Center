    ...
    if (filter_2 === '') {
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [Category],  [brand_name] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel_subChannel_category_brand] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear], [Category],  [brand_name] order by [MonthYear] desc`
            } else {
        ...
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [Category],  [brand_name] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear], [Category],  [brand_name] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [Category],  [brand_name] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear], [Category],  [brand_name] order by [MonthYear] desc`
            }
        }else {
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [Category],  [brand_name] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] in (${channel}) and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear], [Category],  [brand_name] order by [MonthYear] desc`
            } else {
...
let getDeepDivePageData = async (req, res) =>{
    ...
    try {
        ...
        for (let i in all_data) {
            ...
            let categories_data_rt_cy = await getQueryData(connnection, channel_query_rt_cy)
            // let categories_data_rt_cy = await sequelize.query(channel_query_rt_cy)
            connnection = await getConnection()
            let categories_data_rt_py = await getQueryData(connnection, channel_query_rt_py)
            ...
            for(let i in categories_data_rt_cy){
                cy_key[`${categories_data_rt_cy[i]['MonthYear']}/${categories_data_rt_cy[i]['Category']}/${categories_data_rt_cy[i]['brand_name']}`] = categories_data_rt_cy[i]['Retailing_Sum']
            }
            ...
            for(let i in categories_data_rt_py){
                ...
                if(!cy_key[`${monthYear}/${categories_data_rt_py[i]['Category']}/${categories_data_rt_py[i]['brand_name']}`]){
                    let obj = {
                        'Retailing_Sum': 0,
                        'MonthYear': monthYear,
