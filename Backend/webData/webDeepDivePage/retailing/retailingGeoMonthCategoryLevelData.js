    ...
    if (filter_2 === '') {
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [Category] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel_subChannel_category] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [Category] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [Category] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel_subChannel_category] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [Category] order by [MonthYear] desc`
            }
        }else{
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [Category] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel_subChannel_category] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] in (${channel}) and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [Category] order by [MonthYear] desc`
            } else {
    ...
    try {
        ...
        for (let i in all_data) {
            ...
            for(let i in categories_data_rt_cy){
                cy_key[`${categories_data_rt_cy[i]['MonthYear']}/${categories_data_rt_cy[i]['Category']}`] = categories_data_rt_cy[i]['Retailing_Sum']
            }
            ...
            for(let i in categories_data_rt_py){
                ...
                if(!cy_key[`${monthYear}/${categories_data_rt_py[i]['Category']}`]){
                    let obj = {
                        'Retailing_Sum': 0,
                        'MonthYear': monthYear,
            ...
            for(let i in cmDataSetList){
                let cm_data_obj ={
                    ...
                    "category": cmDataSetList[i]["category"],
                }
            ...
            for(let i in p1mDataSetList){
                let p1m_data_obj ={
                    ...
                    "category": p1mDataSetList[i]["category"],
                }
            ...
            for(let i in p3mDataSetList){
                let p3m_data_obj ={
                    ...
                    "category": p3mDataSetList[i]["category"],
                }
            ...
            for(let i in p6mDataSetList){
                let p6m_data_obj ={
                    ...
                    "category": p6mDataSetList[i]["category"],
                }
            ...
            for(let i in p12mDataSetList){
                let p12m_data_obj ={
                    ...
                    "category": p12mDataSetList[i]["category"],
                }
            ...
            for(let i in pyDataSetList){
                let py_data_obj ={
                    ...
                    "category": pyDataSetList[i]["category"],
                }
