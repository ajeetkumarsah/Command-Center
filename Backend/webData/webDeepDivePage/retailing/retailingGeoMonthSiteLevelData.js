...
function getFinalDataOfPNMSet(cy_data, py_data, filter_1, filter_2, channel_list){
    ...
    for(let i in cy_data){
        ...
        if(filter_1 ['filter_key'] === 'division' || filter_1['filter_key'] === 'allIndia'){
            ...
            obj['cy_retailing_sum'] = getFormatedNumberValue_RT(parseInt((obj['cy_retailing_sum'].toFixed(2)).split(".")[0]))
            obj['py_retailing_sum'] = getFormatedNumberValue_RT(parseInt((obj['py_retailing_sum'].toFixed(2)).split(".")[0]))
            mergedArr.push(obj)
        }
    ...
    if (filter_2 === '') {
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            }
        }else{
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            }
        }
        ...
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            }
        }else {
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName] order by [MonthYear] desc`
            }
        }
    }
