...
async function getTableData(bodyData){
    ...
    for (let i in all_data) {
        ...
        if (filter_2 === '') {
            if(channel.length === 0){
                if (filter_1['filter_data'] === 'allIndia') {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and Division in ('N-E', 'S-W')`
                } else {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and [${filter_1['filter_key']}] = '${filter_1['filter_data']}'`
                }
            }else {
                if (filter_1['filter_data'] === 'allIndia') {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and Division in ('N-E', 'S-W') and [ChannelName] in (${channel})`
                } else {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [ChannelName] in (${channel})`
                }
            }
            ...
            if(channel.length === 0){
                if (filter_1['filter_data'] === 'allIndia') {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' `
                } else {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' `
                }
            }else {
                if (filter_1['filter_data'] === 'allIndia') {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [ChannelName] in (${channel})`
                } else {
                    sql_query_no_of_fb_current_year = `select SUM([FB Points achieved]) as fb_achieve_sum , SUM([FB Target]) as fb_target_sum FROM [dbo].[tbl_command_center_fb_new2] where [Calendar Month] = '${calendar_month_cy}' and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [ChannelName] in (${channel})`
                }
            }
