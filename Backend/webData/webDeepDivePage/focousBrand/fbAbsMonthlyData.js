        ...
        if(key === arr[i]['filter_key']){
            for(let j in arr[i]['Branch']){
                if(branch_name === arr[i]['Branch'][j]['filter_key']){
                    return arr[i]['Branch'][j]['fb_achieve_sum']
                }
            }
        }
                ...
                if(branch_name === arr[i]['Branch'][j]['filter_key']){
                    for(let k in arr[i]['Branch'][j]['Channel']){
                        if(channel_name === arr[i]['Branch'][j]['Channel'][k]['filter_key']){
                            return arr[i]['Branch'][j]['Channel'][k]['fb_achieve_sum']
                        }
                    }
                }
            }
                        ...
                        if(channel_name === arr[i]['Branch'][j]['Channel'][k]['filter_key']){
                            for(let l in arr[i]['Branch'][j]['Channel'][k]['SubChannel']){
                                if(subChannel_name === arr[i]['Branch'][j]['Channel'][k]['SubChannel'][l]['filter_key']){
                                    return arr[i]['Branch'][j]['Channel'][k]['SubChannel'][l]['fb_achieve_sum']
                                }
                            }
                        }
                    }
...
async function getTableData (bodyData){
    ...
    try {
        ...
        for (let i in all_data) {
                ...
                for (let i in mergedArr) {
                    if (current_month === mergedArr[i]['Calendar Month']){
                        ...
                        if (key in DivisionObj) {
                            ...
                            // DivisionObj[`${key}`]['fb_achieve_sum'] = parseFloat(((DivisionObj[`${key}`]['fb_per'] + (((mergedArr[i]['fb_achieve_sum'])/(mergedArr[i]['fb_target_sum'])) * 100))/2).toFixed(2))
            ...
            mergeObjAllIndia['fb_abs_1'] = final_result[0][0]['fb_achieve_sum']
            mergeObjAllIndia['fb_abs_2'] = final_result[0][1]['fb_achieve_sum']
            mergeObjAllIndia['fb_abs_3'] = final_result[0][2]['fb_achieve_sum']
            ...
            for (let i in final_result[0][0]['division']) {
                ...
                if (!(div_key in mergeObjAllDivision)) {
                    mergeObjAllDivision[`${final_result[0][0]['division'][i]['filter_key']}`] = {
                        ...
                        'fb_abs_1': final_result[0][0]['division'][i] ? final_result[0][0]['division'][i]['fb_achieve_sum'] : 0,
                        'fb_abs_2': final_result[0][1]['division'][i] ? final_result[0][1]['division'][i]['fb_achieve_sum'] : 0,
                        'fb_abs_3': final_result[0][2]['division'][i] ? final_result[0][2]['division'][i]['fb_achieve_sum'] : 0,
                        'site': []
                    }
                }
                ...
                for (let j in final_result[0][0]['division'][i]['Site']) {
                    ...
                    let fb_abs_1 = final_result[0][0]['division'][i] ? getMatchedValue(final_result[0][0]['division'][i]['Site'], site_key) : 0
                    let fb_abs_2 = final_result[0][1]['division'][i] ? getMatchedValue(final_result[0][1]['division'][i]['Site'], site_key) : 0
                    let fb_abs_3 = final_result[0][2]['division'][i] ? getMatchedValue(final_result[0][2]['division'][i]['Site'], site_key) : 0
                    mergeObjSite = {
                        'filter_key': site_key,
                        'fb_abs_1': fb_abs_1,
                        'fb_abs_2': fb_abs_2,
                    ...
                    for (let k in final_result[0][0]['division'][i]['Site'][j]['Branch']) {
                        ...
                        let fb_abs_1 = final_result[0][0]['division'][i] ? getMatchedBranchValue(final_result[0][0]['division'][i]['Site'], site_key, branch_key) : 0
                        let fb_abs_2 = final_result[0][1]['division'][i] ? getMatchedBranchValue(final_result[0][1]['division'][i]['Site'], site_key, branch_key) : 0
                        let fb_abs_3 = final_result[0][2]['division'][i] ? getMatchedBranchValue(final_result[0][2]['division'][i]['Site'], site_key, branch_key) : 0
                        mergeObjBranch = {
                        ...
                        for (let l in final_result[0][0]['division'][i]['Site'][j]['Branch'][k]['Channel']) {
                            ...
                            let fb_abs_1 = final_result[0][0]['division'][i] ? getMatchedChannelValue(final_result[0][0]['division'][i]['Site'], site_key, branch_key, channel_key) : 0
                            let fb_abs_2 = final_result[0][1]['division'][i] ? getMatchedChannelValue(final_result[0][1]['division'][i]['Site'], site_key, branch_key, channel_key) : 0
                            let fb_abs_3 = final_result[0][2]['division'][i] ? getMatchedChannelValue(final_result[0][2]['division'][i]['Site'], site_key, branch_key, channel_key) : 0
                            mergeObjChannel = {
                            ...
                            for (let m in final_result[0][0]['division'][i]['Site'][j]['Branch'][k]['Channel'][l]['SubChannel']) {
                                ...
                                let fb_abs_1 = final_result[0][0]['division'][i] ? getMatchedSubChannelValue(final_result[0][0]['division'][i]['Site'], site_key, branch_key, channel_key, sub_channel_key) : 0
                                let fb_abs_2 = final_result[0][1]['division'][i] ? getMatchedSubChannelValue(final_result[0][1]['division'][i]['Site'], site_key, branch_key, channel_key, sub_channel_key) : 0
                                let fb_abs_3 = final_result[0][2]['division'][i] ? getMatchedSubChannelValue(final_result[0][2]['division'][i]['Site'], site_key, branch_key, channel_key, sub_channel_key) : 0
                                mergeObjSubChannel = {
...
let getDeepDivePageData = async (req, res) => {
    try {
        ...
        let cacheKey = 'fbMonthlyAbsDeepDiveData'
        let final_result = []
