...
let getDeepDivePageDataBySBF = async (req, res) =>{
    ...
    try {
        ...
        let categories_data_fb = await getQueryData(connection, category_query_fb)
        // let categories_data_fb = await sequelize.query(category_query_fb)

        let categorySet = {}
        for(let i in categories_data_fb){
            let data = sanitizeInput(categorySet[`${categories_data_fb[i]['CategoryName']}`])
            if(data){
                data['fb_target'] = data['fb_target'] + categories_data_fb[i]['fb_target_sum']
                data['fb_achieved'] = data['fb_achieved'] + categories_data_fb[i]['fb_achieve_sum']
                data['fb_IYA'] = (data['fb_IYA'] + ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100))/2
            }
            else{
                categorySet[`${categories_data_fb[i]['CategoryName']}`] = {
                    'fb_target':  categories_data_fb[i]['fb_target_sum'],
                    'fb_achieved': categories_data_fb[i]['fb_achieve_sum'],
                    'fb_IYA': ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100)
                }
            }
        }
        let brandObj = {}
        for(let i in categories_data_fb){
            let obj = {}
            let key  = sanitizeInput(categories_data_fb[i]['CategoryName'])
            if (key in brandObj){
                if(brandObj[`${categories_data_fb[i]['BrandName']}`]){
                    brandObj[`${categories_data_fb[i]['BrandName']}`] = categories_data_fb[i]['BrandName']
                    brandObj[`${categories_data_fb[i]['fb_achieved']}`] += categories_data_fb[i]['fb_achieve_sum']
                    brandObj[`${categories_data_fb[i]['fb_target']}`] += categories_data_fb[i]['fb_target_sum']
                    brandObj[`${categories_data_fb[i]['fb_IYA']}`] = (brandObj[`${categories_data_fb[i]['fb_IYA']}`] + ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100))/2
                    brandObj[`${categories_data_fb[i]['BrandForm']}`] = []
                }else {
                    obj['BrandName'] = categories_data_fb[i]['BrandName']
                    obj['fb_achieved'] = categories_data_fb[i]['fb_achieve_sum']
                    obj['fb_target'] = categories_data_fb[i]['fb_target_sum']
                    obj['fb_IYA'] = ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100)
                    obj['BrandForm'] = []
        ...
        for(let i in categories_data_fb){
            let obj = {}
            let key  = categories_data_fb[i]['BrandName']
            if (key in brandForm){

                if(brandForm[`${categories_data_fb[i]['BrandForm']}`]){
                    brandForm[`${categories_data_fb[i]['BrandForm']}`] = categories_data_fb[i]['BrandForm']
                    brandForm[`${categories_data_fb[i]['fb_achieved']}`] += categories_data_fb[i]['fb_achieve_sum']
                    brandForm[`${categories_data_fb[i]['fb_target']}`] += categories_data_fb[i]['fb_target_sum']
                    brandForm[`${categories_data_fb[i]['fb_IYA']}`] = (brandForm[`${categories_data_fb[i]['fb_IYA']}`] + ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100))/2
                    brandForm[`${categories_data_fb[i]['SBF']}`] = []
                }else {
                    obj['BrandForm'] = categories_data_fb[i]['BFName']
                    obj['fb_achieved'] = categories_data_fb[i]['fb_achieve_sum']
                    obj['fb_target'] = categories_data_fb[i]['fb_target_sum']
                    obj['fb_IYA'] = ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100)
                    obj['SBF'] = []
        ...
        for(let i in categories_data_fb){
            let obj = {}
            let key  = categories_data_fb[i]['BFName']
            if (key in SBForm){

                if(SBForm[`${categories_data_fb[i]['SBForm']}`]){
                    SBForm[`${categories_data_fb[i]['SBForm']}`] = categories_data_fb[i]['SBForm']
                    SBForm[`${categories_data_fb[i]['fb_achieved']}`] += categories_data_fb[i]['fb_achieve_sum']
                    SBForm[`${categories_data_fb[i]['fb_target']}`] += categories_data_fb[i]['fb_target_sum']
                    SBForm[`${categories_data_fb[i]['fb_IYA']}`] = (SBForm[`${categories_data_fb[i]['fb_IYA']}`] + ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100))/2
                }else {
                    obj['SBForm'] = categories_data_fb[i]['SBFName']
                    obj['fb_achieved'] = categories_data_fb[i]['fb_achieve_sum']
                    obj['fb_target'] = categories_data_fb[i]['fb_target_sum']
                    obj['fb_IYA'] = ((categories_data_fb[i]['fb_achieve_sum'])/(categories_data_fb[i]['fb_target_sum']) * 100)
                    SBForm[key].push(obj)
