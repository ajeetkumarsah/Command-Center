    ...
    if (filter_2 === '') {
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [channel_name] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [channel_name] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [channel_name] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [channel_name] order by [MonthYear] desc`
            }
        }else{
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [channel_name] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] in (${channel}) and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [channel_name] order by [MonthYear] desc`
            } else {
        ...
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [channel_name] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [channel_name] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [channel_name] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [channel_name] order by [MonthYear] desc`
            }
        }else {
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [channel_name] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] in (${channel}) and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [channel_name] order by [MonthYear] desc`
            } else {
