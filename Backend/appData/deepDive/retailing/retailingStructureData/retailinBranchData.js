...
function getTableName(data, table_name){
    if(data === '[Category]'){table_name = table_name+"category"}
    if(data === '[brand_name]'){table_name = table_name+"Brand"}
    if(data === '[brandform_name]'){table_name = table_name+"BF"}
    if(data === '[sbf_name]'){table_name = table_name+"SBF"}
    return table_name
}
...
function getPNMSet(data, monthsList, no_of_months, skip){
    ...
    for(let i in mergeArr){
        ...
        let branchName = sanitizeInput(mergeArr[i]['BranchName']);
        let key = `${division}/${siteName}/${branchName}`;
        if(obj[key]){
            ...
            obj[key] = {
                ...
                'SiteName': mergeArr[i]['SiteName'],
                'BranchName': mergeArr[i]['BranchName']
            }
        }
    }
    let final_list = []
...
function getQuery(calendar_month, filter_2, filter_1, channel, site){
    let channel_query_rt_cy = ''
    let geo_filter = filter_1['filter_key'] === 'allIndia' ? 'division' : filter_1['filter_key']
    if (filter_2 === '') {
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [SiteName] = '${site}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [SiteName] = '${site}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            }
        }else{
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [SiteName] = '${site}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[tbl_command_center_rt_Division_cluster_site_branch_channel] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [SiteName] = '${site}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            }
        }
        ...
        if(channel === ''){
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [SiteName] = '${site}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [SiteName] = '${site}' and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            }
        }else {
            if (filter_1['filter_data'] === 'allIndia') {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [SiteName] = '${site}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            } else {
                channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear], [${geo_filter}], [SiteName], [BranchName] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}' and [SiteName] = '${site}' and [channel_name] in (${channel}) group by [MonthYear], [${geo_filter}], [SiteName], [BranchName] order by [MonthYear] desc`
            }
        }
    }
...
let getDeepDivePageData = async (req, res) =>{
    ...
    try {
        ...
        for (let i in all_data) {
            ...
            let channel_query_rt_cy = getQuery(calendar_month_cy, filter_2, filter_1, channel, site)
            let channel_query_rt_py = getQuery(calendar_month_py, filter_2, filter_1, channel, site)
