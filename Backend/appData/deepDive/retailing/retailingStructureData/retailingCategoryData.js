...
function getColName(filter){
    let colName = ''
    if(filter === 'category'){
        colName = 'Category'
    }else if(filter === 'brand'){
        colName = 'brand_name'
    }else if(filter === 'brandForm'){
        colName = 'brandform_name'
    }else if(filter === 'subBrandForm'){
        colName = 'sbf_name'
    }
    return colName
}

function getTableName(table_name, filter){
...
function getQuery(calendar_month, filter_2, filter_1, channel, site, branch, filter){
    ...
    // if (filter_2 === '') {
    let table_name = 'tbl_command_center_rt_Division_cluster_site_branch_channel_subChannel_'
    let colName = ''
    colName = getColName(filter)
    table_name = getTableName(table_name, filter)
    if(channel === ''){
        if (filter_1['filter_data'] === 'allIndia') {
            channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [${colName}] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [${colName}] order by [MonthYear] desc`
        } else {
            channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [${colName}] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [${colName}] order by [MonthYear] desc`
        }
    }else{
        if (filter_1['filter_data'] === 'allIndia') {
            channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [${colName}] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W')  and [channel_name] in (${channel}) and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [${colName}] order by [MonthYear] desc`
        } else {
    ...
    // } else {
    //     let table_name = 'tbl_command_center_rt_Division_cluster_site_branch_channel_subChannel_category'
    //     // table_name = getTableName(filter_2['filter_key'], table_name)
    //     if(channel === ''){
    //         if (filter_1['filter_data'] === 'allIndia') {
    //             channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [Category] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [Category] order by [MonthYear] desc`
    //         } else {
    //             channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [Category] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and [${filter_1['filter_key']}] = '${filter_1['filter_data']}' and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] not in ('Aggregator SubD', 'SubD', 'SubD A', 'SubD B') and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [Category] order by [MonthYear] desc`
    //         }
    //     }else {
    //         if (filter_1['filter_data'] === 'allIndia') {
    //             channel_query_rt_cy = `select sum([Retailing]) as Retailing_Sum, [MonthYear],  [Category] from [dbo].[${table_name}] where [MonthYear] in (${calendar_month}) and Division in ('N-E', 'S-W') and [${filter_2['filter_key']}] = '${filter_2['filter_data']}'  and [channel_name] in (${channel}) and [SiteName] like '${siteName}' and [BranchName] like '${branchName}' group by [MonthYear],  [Category] order by [MonthYear] desc`
    //         } else {
...
let getDeepDivePageData = async (req, res) =>{
    ...
    try {
        ...
        for (let i in all_data) {
            ...
            let channel_query_rt_cy = getQuery(calendar_month_cy, filter_2, filter_1, channel, site, branch, filter)
            let channel_query_rt_py = getQuery(calendar_month_py, filter_2, filter_1, channel, site, branch, filter)
            ...
            let colName = getColName(filter)
            let cy_key = {}
            for(let i in categories_data_rt_cy){
                cy_key[`${categories_data_rt_cy[i]['MonthYear']}/${categories_data_rt_cy[i][colName]}`] = categories_data_rt_cy[i]['Retailing_Sum']
            }
            ...
            for(let i in categories_data_rt_py){
                ...
                if(!cy_key[`${monthYear}/${categories_data_rt_py[i][colName]}`]){
                    let obj = {
                        'Retailing_Sum': 0,
                        'MonthYear': monthYear
                    }
                    obj[colName] = categories_data_rt_py[i][colName]
            ...
            let cmDataSetList = getFinalDataOfPNMSet(cm_set_cy, cm_set_py, filter_1, filter_2, channel_list, colName)
            for(let i in cmDataSetList){
                ...
                cm_data_obj[colName] = cmDataSetList[i][colName]
            ...
            let p1mDataSetList = getFinalDataOfPNMSet(p1m_set_cy, p1m_set_py, filter_1, filter_2, channel_list, colName)
            for(let i in p1mDataSetList){
                ...
                p1m_data_obj[colName] = p1mDataSetList[i][colName]
            ...
            let p3mDataSetList = getFinalDataOfPNMSet(p3m_set_cy, p3m_set_py, filter_1, filter_2, channel_list, colName)
            for(let i in p3mDataSetList){
                ...
                p3m_data_obj[colName] = p3mDataSetList[i][colName]
            ...
            let p6mDataSetList = getFinalDataOfPNMSet(p6m_set_cy, p6m_set_py, filter_1, filter_2, channel_list, colName)
            for(let i in p6mDataSetList){
                ...
                p6m_data_obj[colName] = p6mDataSetList[i][colName]
            ...
            let p12mDataSetList = getFinalDataOfPNMSet(p12m_set_cy, p12m_set_py, filter_1, filter_2, channel_list, colName)
            for(let i in p12mDataSetList){
                ...
                p12m_data_obj[colName] = p12mDataSetList[i][colName]
            ...
            let pyDataSetList = getFinalDataOfPNMSet(py_set_cy, py_set_py, filter_1, filter_2, channel_list, colName)
            for(let i in pyDataSetList){
                ...
                py_data_obj[colName] = pyDataSetList[i][colName]
                final_obj['financial_year'].push(py_data_obj)
            }
            all_result.push([final_obj])
        }
        return ({successful: true, data: all_result});
    } catch (e) {
        console.log('error',e)
        return ({successful: false, error: 'An internal server error occurred.'})
    }
}
