...
import 'login/fedauth_login.dart';
    ...
    return Scaffold(
      ...
      body: Stack(children: [
        Container(
          ...
          child: Lottie.asset('assets/json/splashbglottie.json', width:MediaQuery.of(context).size.width, fit: BoxFit.fill,),
          // Column(
    ...
    void startTimer() {
      Timer(const Duration(seconds: 5), () {
        ...
        if (seen) {
          ...
          if (code != null) {
            LoginController().getAccessTokenFromCode(code).then((isLoggedIn) {
              if (isLoggedIn) {
                ...
                // Navigator.pushReplacementNamed(context, RoutesName.pglogin);
                Navigator.pushReplacement(
                    context,
                    MaterialPageRoute(
                        builder: (BuildContext context) => const SelectProfile()));
              }
            });
          } else {
            // Navigator.pushReplacementNamed(context, RoutesName.pglogin);
          ...
          Navigator.pushReplacement(context,
              MaterialPageRoute(builder: (BuildContext context) => const IntroScreen()));
        }
      });
